...
!DEC$ IF DEFINED (NETCDF)
  ...
  PUBLIC :: cdfr_4i, cdfr_4d, cdfr_3i, cdfr_3l, cdfr_3d, cdfr_3c16, cdfr_3f, cdfr_3c8,           &
  ...
  INTERFACE cdf_read
     MODULE PROCEDURE cdfr_4i, cdfr_4d, cdfr_4f, cdfr_3i, cdfr_3l, cdfr_3d, cdfr_3c16, cdfr_3f, cdfr_3c8,           &
                      cdfr_2i, cdfr_2l, cdfr_2d, cdfr_2c16, cdfr_2f, cdfr_2c8, cdfr_2c,  &
                      cdfr_1i, cdfr_1l, cdfr_1d, cdfr_1c16, cdfr_1f, cdfr_1c8, cdfr_1c,  &
...
SUBROUTINE cdfr_4i(ncid,varnam,varval,ier)
  ...
  ! Local
  integer, dimension(4)   :: st,     cnt,    ldim
  ...
  !
  if(dimlens(1)==ldim(1).and.dimlens(2)==ldim(2).and.dimlens(3)==ldim(3).and.dimlens(4)==ldim(4))then
     status = nf_get_var_int(ncid, varid, varval)
     if(status/=NF_NOERR) then
        call handle_err(status,varnam,'cdfr_4i', 'nf_get_var_int')
        return
     endif
  else
     ...
     ndim3 = min(dimlens(3), ldim(3))
		 ndim4 = min(dimlens(4), ldim(4))
     varval(1:ndim1, 1:ndim2, 1:ndim3, 1:ndim4) = temp(1:ndim1, 1:ndim2, 1:ndim3, 1:ndim4)
     deallocate(temp)
  endif
...
END SUBROUTINE cdfr_4i

SUBROUTINE cdfr_4d(ncid,varnam,varval,ier)
  implicit none
  ! Input
  integer,       intent(in) :: ncid
  character*(*), intent(in) :: varnam
  ...
  if(dimlens(1)==ldim(1).and.dimlens(2)==ldim(2).and.dimlens(3)==ldim(3).and.dimlens(4)==ldim(4))then
     ...
     if(status/=NF_NOERR) then
        call handle_err(status,varnam,'cdfr_4d', 'nf_get_var_double')
        return
     endif
  else
     allocate(temp(dimlens(1), dimlens(2), dimlens(3), dimlens(4)))
...
!!$  end do
  ...
  if (PRESENT (ier)) ier = 0
END SUBROUTINE cdfr_4d

SUBROUTINE cdfr_4f(ncid,varnam,varval,ier)
  implicit none
  ! Input
  integer,       intent(in) :: ncid
  character*(*), intent(in) :: varnam
  ...
  if(dimlens(1)==ldim(1).and.dimlens(2)==ldim(2).and.dimlens(3)==ldim(3).and.dimlens(4)==ldim(4))then
     ...
     if(status/=NF_NOERR) then
        call handle_err(status,varnam,'cdfr_4f', 'nf_get_var_real')
        return
     endif
  else
     allocate(temp(dimlens(1), dimlens(2), dimlens(3), dimlens(4)))
