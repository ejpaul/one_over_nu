...
!DEC$ IF DEFINED (NETCDF)
  ...
  PUBLIC :: cdfw_4i, cdfw_4d, cdfw_4f, cdfw_3i, cdfw_3l, cdfw_3d, cdfw_3c16, cdfw_3f, cdfw_3c8,  &
  ...
  INTERFACE cdf_write
     MODULE PROCEDURE cdfw_4i, cdfw_4d, cdfw_4f, cdfw_3i, cdfw_3l, cdfw_3d, cdfw_3c16, cdfw_3f, cdfw_3c8, &
          cdfw_2i, cdfw_2l, cdfw_2d, cdfw_2c16, cdfw_2f, cdfw_2c8, cdfw_2c, &
          cdfw_1i, cdfw_1l, cdfw_1d, cdfw_1c16, cdfw_1f, cdfw_1c8, cdfw_1c, &
          cdfw_0i, cdfw_0l, cdfw_0d, cdfw_0c16, cdfw_0f, cdfw_0c8
  END INTERFACE
...
!cdfPutVar implementation routines
  ...
  SUBROUTINE cdfw_4i(ncid,varnam,varval,ier)
    !     Write 4 dimensional Integer data array
    ...
    if (sts .ne. 0) return
    if (ndims .ne. 4) then
       print "('% cdfPutVar_4i: --E-- The variable ',a,               &
            &         ' was defined as',i2,' dimensional')",varnam,ndims
       return
    end if
    sts = nf_put_vara_int(ncid,varid,st,dimlens,varval)
    call handle_err(sts,varnam,'cdfPutVar_4i','nf_put_vara_int')
    if (PRESENT (ier)) ier = sts
  END SUBROUTINE cdfw_4i
...
SUBROUTINE cdfw_4f(ncid,varnam,varval,ier)
    !     Write 4 dimensional default Real data array
    !     Use cdfDefVar to define the Variable
    implicit none
    ! Input
    integer,                         intent(in) :: ncid
    ...
    if (sts .ne. 0)  return
    if (ndims .ne. 4) then
       print "('% cdfPutVar_4f: --E-- The variable ',a,               &
            &         ' was defined as',i2,' dimensional')",varnam,ndims
       return
    end if
    sts = nf_put_vara_real(ncid,varid,st,dimlens,varval)
    call handle_err(sts,varnam,'cdf_4f','nf_put_vara_real')
    if (PRESENT (ier)) ier = sts
  END SUBROUTINE cdfw_4f
...
10     format('% cdfDefVar --E--  Rank',i3,' not supported for ',a)
  ...
  SUBROUTINE cdfd_4i(ncid,varnam,varval,ier,dimname)
    !     define 4 dimensional Integer data array
    implicit none
!!$  include "netcdf.inc"
    ! Input
    integer,                   intent(in) :: ncid
...
!!$  include "netcdf.inc"
  ...
  SUBROUTINE cdfd_4f(ncid,varnam,varval,ier,dimname)
    !     define 4 dimensional real data array
    implicit none
!!$  include "netcdf.inc"
    ! Input
    integer,                   intent(in) :: ncid
